...
**Resolved (2026-09-22):** the WPP licence terms, checked by the user. The terms were previously unverifiable from this repository — the site is a JavaScript SPA and served no fetchable terms text — and the attribution shipped on the strength of CC BY 3.0 IGO being likely. It is no longer a pre-release item.
...
- **Resolved (2026-09-22) — WPP reference year: always the current year.** `DEFAULT_YEAR` in `scripts/data/refresh.ts` is `new Date().getUTCFullYear()` rather than a pinned number. The game asks about the present; a pinned year is wrong from the moment nobody remembers to move it, and stale populations are wrong in a way no gate in this project can see. The cost is accepted and visible: on the 1st of January the diff is all 195 countries, `data-drift.yml` opens an issue, and the report names the rollover — `reference year moved: 2026 -> 2027` — so a calendar change cannot be mistaken for a republished WPP revision. `tests/unit/data-pipeline.test.ts` pins its own fixtures to `FIXTURE_YEAR` so the offline suite does not fail on New Year's Day for reasons unrelated to the code it tests.
...
- **Resolved (2026-09-22) — repo size budget: none.** Declined. `flags.json` (1.5 MB), 195 PNGs, a 1.9 MB baseline HTML fixture, a 787 KB mp3, `data/raw/` and the golden fixtures are the cost of a project whose oracles are committed artifacts, and nothing about the current size is awkward enough to trade any of them for.
- **Resolved (2026-09-22) — the demo mp3: regenerated.** It had been a stale v6 render kept as a historical artifact. The objection to porting its generator was that it would put an ffmpeg-conditional path into the deploy-critical unit; regenerating it *after* U14, as a script wired into nothing, costs that nothing. `docs/provenance/manifest-sha256.json` keeps the v6 hash under the path v7 shipped it at, because that file records what v7 was.
