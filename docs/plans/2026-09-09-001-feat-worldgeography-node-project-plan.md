...
- **Blocking before U11 — mid-run language switching.** Disable the switcher during an active run, or prompt to abandon the run? The plan assumes disable; either is defensible, but whichever is chosen needs copy in both locales, and shipping neither means shipping mixed-language questions.
