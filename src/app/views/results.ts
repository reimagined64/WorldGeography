...
export function renderResults(): void {
  ...
  if(optional('review'))$('review').onclick=()=>{
    const questions=wrong.map(({q},i)=>({...Core.makeQuestion(byCode[q.country]!,q.type,countries,questionBundle(),game.options.difficulty),visit:i,player:0}));
