...
import {
  $, attempts, coords, dialog, esc, flagImage, lifePips, optional, persist, pointText,
...
export function questionHeader(q: Question): string {
  ...
  const bonusProgress=bonus?t('head.bonusNow'):pending?(pending>1?t('head.bonusWaitingMany',{count:pending}):t('head.bonusWaiting')):t('head.bonusProgress',{points:pointText(Math.max(0,remaining))});
  const caption=game.review?t('head.captionReview',{index:game.index+1,total:game.questions.length}):bonus?t('head.captionBonus',{points:pointText(q.bonusThreshold!)}):t('head.captionQuestion',{number:String(game.index+1).padStart(2,'0')});
  return `<div class="question-head"><span>${caption}</span><strong>${esc(t('game.turn',{name:game.options.names[q.player]||t('game.playerFallback',{number:q.player+1})}))}</strong></div><div class="steps" aria-label="${bonus?t('head.stepsBonus'):t('head.stepsRound',{number:step+1})}">${(bonus?['flag']:Core.TYPES).map((_,i)=>`<span class="step ${bonus?'current bonus-step':i<step?'done':i===step?'current':''}"></span>`).join('')}</div>${game.review?'':`<div class="question-lives"><span>${lifePips(lives!)} <b>${lives}</b> ${attempts(lives!).replace(/^\d+\s/,'')}</span><span id="bonus-progress" data-next-bonus="${threshold}" title="${esc(t('head.bonusTitle',{points:pointText(threshold)}))}">${bonusProgress}</span></div>`}`;
}
...
export function timerPanel(answer: AnswerResult | null | undefined = null): string {
  ...
  return `<div class="timing-panel ${answer?'finished':''}" id="timing-panel"><div class="time-readout"><span class="instrument-label">${!limit?t('timer.labelReview'):pending?t('timer.labelPending'):paused?t('timer.labelPaused'):answer?t('timer.labelLeft'):t('timer.labelRunning')}</span><strong><span data-time-readout>${limit?seconds(remaining):'∞'}</span><small>${limit?'s':''}</small></strong></div><div class="reward-readout"><span class="instrument-label">${answer?t('timer.rewardEarned'):t('timer.rewardPlaying')}</span><strong><span data-points-readout>${pointText(pts)}</span><small>${t('timer.points')}</small></strong></div>${store.phase==='question'?`<button id="pause-game" class="pause-button" aria-label="${esc(t('timer.pauseLabel'))}" title="${esc(t('timer.pauseTitle'))}">Ⅱ</button>`:''}<div class="time-track" id="time-track" role="progressbar" aria-label="${esc(t('timer.trackLabel'))}" aria-valuemin="0" aria-valuemax="${limit/1000||1}" aria-valuenow="${limit?Math.ceil(remaining/1000):1}"><span id="time-fill" style="transform:scaleX(${limit?remaining/limit:1})"></span></div><div class="timing-note">${!limit?t('timer.noteReview'):pending?t('timer.notePending'):answer?t('timer.noteAnswered'):paused?t('timer.notePaused'):t('timer.noteRunning')}</div></div>`;
}

export function footer(): string {
  const game=store.game!;
  return `<div class="game-footer"><span class="coord">${store.phase==='flying'?t('footer.flying'):store.phase==='paused'?t('footer.paused'):store.phase==='feedback'?t('footer.feedback'):game.review?t('footer.review'):t('footer.playing')}</span><button id="back-home" class="text-button">${t('footer.leave')}</button></div>`;
}
...
export function renderGame(): void {
  ...
  if(store.phase==='paused'){
    $('side-panel').innerHTML=questionHeader(q)+timerPanel()+`<div class="paused-card"><span class="pause-emblem" aria-hidden="true">Ⅱ</span><h2>${t('pause.title')}</h2><p>${t('pause.body')}</p><button id="resume-clock" class="primary"><span>${t('pause.resume')}</span><span aria-hidden="true">→</span></button><p class="pause-shortcut">${t('pause.shortcut')}</p></div>`+footer();
  ...
  if(answer&&!game.review){
    if(answer.scoreLifeDelta)lifeParts.push(t('life.milestone',{thresholds:answer.milestoneThresholds.map(pointText).join(' / '),gained:attempts(answer.scoreLifeDelta),next:bonus?t('life.milestoneNextBonus'):t('life.milestoneNextCountry')}));
    if(answer.flagLifeDelta)lifeParts.push(t('life.flagEarned'));
    if(!answer.correct)lifeParts.push(bonus?t('life.bonusMissed'):t('life.noPenalty'));
    lifeParts.push(t('life.remaining',{attempts:attempts(answer.livesAfter)})+(answer.livesAfter===0&&!game.gameOver&&!bonus&&Core.TYPES.indexOf(q.type as QuestionType)<4?t('life.finishCountry'):''));
  }
  const lifeMessage=lifeParts.join(' ');
  const turn=game.review?null:Core.nextTurn(game);
  const nextLabel=end?t('next.results'):game.review?t('next.question'):turn&&turn.kind==='bonus'?t('next.bonus'):turn&&turn.kind==='country'?(turn.player!==q.player?t('next.handOver'):t('next.country')):bonus?t('next.sameCountry'):t('next.question');
  $('side-panel').innerHTML=questionHeader(q)+timerPanel(answer)+`<div class="question-category"><span class="category-number">${bonus?'+1':step+1}</span>${esc(kindName(q.type))}</div><h2 id="question-title" class="question-title" tabindex="-1">${esc(q.prompt)}</h2>
  ${bonus?`<div class="bonus-flag-card">${flagImage(c.code,!answer,'bonus-flag')}<div class="bonus-reward">${game.review?t('bonus.rewardReview'):t('bonus.reward')}</div></div>`:''}
  ...
  ${answer?`<div class="feedback" aria-live="polite"><div class="feedback-title ${answer.correct?'':'incorrect'}">${answer.timedOut?t('feedback.timedOut'):answer.correct?t('feedback.correct',{points:pointText(answer.points)}):t('feedback.wrong')}</div>${lifeMessage?`<div class="life-feedback ${answer.lifeDelta>0?'life-earned':answer.lifeDelta<0?'life-lost':''}">${esc(lifeMessage)}${game.gameOver?t('life.allOut'):''}</div>`:''}<div class="answer-timing">${answer.timedOut?t('feedback.noAnswer'):t('feedback.answeredIn',{seconds:seconds(answer.elapsedMs)})}${answer.correct?t('feedback.breakdown',{base:answer.basePoints,bonus:pointText(answer.bonusPoints||0)}):''}</div><p>${esc(q.explanation)}</p>${populationSourceLink(q.source)}<button id="next" class="primary"><span>${nextLabel}</span><span class="arrow" aria-hidden="true">→</span></button></div>`:`<p class="question-tip">${t('question.tip')}</p>`}`+footer();
