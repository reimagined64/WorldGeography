 ...
 *
 * The theme names are Czech string data rather than catalog keys, exactly as
 * v7 wrote them; U11 routes them through the i18n catalog.
 */
...
/** `[name, eight-bar progression, four four-note phrases]`, one per authored theme. */
export type ThemeMaterial = readonly [string, readonly number[], readonly number[]];
...
export function composeTheme(material: ThemeMaterial,theme: number): Score {
  const [name,progression,motif]=material,roots: number[]=[],chords: number[][]=[],lead: number[]=Array<number>(128).fill(0);
...
export const QUESTION_SCORES: readonly Score[]=Object.freeze([Object.freeze({...QUESTION_SCORE,name:'Původní soutěžní motiv'}),...THEME_MATERIAL.map(composeTheme)]);
