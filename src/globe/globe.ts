 ...
 *
 * A structural port of the v7 `globe.js`: same method names, same argument
 * order, same easing and the same four-stage flight, because the twelve-second
 * arrival is the one animation the game's state machine waits on and the saved
 * runs replay across its stage boundaries.
 *
 * Four fields are typed against invariants the checker cannot see. `resize()`
 * runs in the constructor, so `w`, `h` and `dpr` are numbers from the first
 * frame onwards and the size test in `draw` is about a zero-sized canvas rather
 * than an unset one. `rot` is rebuilt by `update` on every frame and `draw`
 * returns early while it is missing, which is what lets the path helpers assert
...
/** Stage lengths in milliseconds. `total` is their sum, and a fixed contract. */
export const FLIGHT=Object.freeze({depart:600,spin:8400,settle:1800,zoom:1200,total:12000,turns:3});

/** A point on the unit sphere, in map space before `rot` and in view space after it. */
...
/** A polygon with its map-space unit vectors precomputed once at construction. */
interface DrawablePolygon extends MapPolygon {
  vec: Vector[];
}

/** What `onProgress` is handed every frame, and what `app.js` renders from. */
...
export class Globe {
  ...
  dpr!: number;
  prev!: [number, number];
  rot: ((v: Vector) => Vector) | undefined;
  radius: number | undefined;
  constructor(canvas: HTMLCanvasElement,polygons: readonly MapPolygon[]){
    // A browser that will not hand back a 2D context cannot draw the game at all.
    this.canvas=canvas;this.ctx=canvas.getContext('2d')!;this.polys=polygons.map(p=>({...p,vec:p.points.map(([lon,lat]): Vector=>{const f=lat*RAD,l=lon*RAD;return [Math.cos(f)*Math.sin(l),Math.sin(f),Math.cos(f)*Math.cos(l)];})}));
    ...
    this.frame=(t: number)=>{if(!document.hidden&&t-this.lastFrame>30){this.lastFrame=t;this.update(t);this.draw(t);if(this.pendingComplete){const done=this.pendingComplete;this.pendingComplete=null;done();}}this.raf=requestAnimationFrame(this.frame);};
  ...
  zoomFor(c: Country): number {
    ...
    // The index is inside a non-empty, freshly sorted array on this branch.
  ...
  pauseFlight(paused: boolean): void {if(this.flight){this.flight.paused=paused;this.previousTick=null;}}
  cancelFlight(): void {this.flight=null;this.pendingComplete=null;this.locked=false;this.targetLat=this.lat;this.targetLon=this.lon;this.targetZoom=this.zoom;}
  ...
  update(t: number): void {
    ...
    if(f){
      ...
      const smooth=(x: number)=>x*x*(3-2*x),easeOut=(x: number)=>1-Math.pow(1-x,3);
      const spinEnd=FLIGHT.depart+FLIGHT.spin,settleEnd=spinEnd+FLIGHT.settle;
      let stage: string,done=f.elapsed>=FLIGHT.total;
      ...
      // The flight is live on this line, so `flightState` cannot return null.
    ...
    this.rot=(v: Vector): Vector=>{const x=v[0]*cl-v[2]*sl,z=v[0]*sl+v[2]*cl;return [x,-(v[1]*cp-z*sp),v[1]*sp+z*cp];};
  }
  point(lon: number,lat: number): Vector {const f=lat*RAD,l=lon*RAD;return this.rot!([Math.cos(f)*Math.sin(l),Math.sin(f),Math.cos(f)*Math.cos(l)]);}
  intersect(a: Vector,b: Vector): Vector {const t=a[2]/(a[2]-b[2]),x=a[0]+(b[0]-a[0])*t,y=a[1]+(b[1]-a[1])*t;const n=Math.hypot(x,y)||1;return[x/n,y/n,0];}
  landPath(vec: Vector[],r: number): boolean {
    const ctx=this.ctx,pts=vec.map(this.rot!),visible=pts.findIndex(p=>p[2]>=0);if(visible<0)return false;
    // `visible` is an index `findIndex` just produced, so both reads below hold.
    const ordered=[...pts.slice(visible),...pts.slice(0,visible),pts[visible]!];
    ctx.beginPath();ctx.moveTo(ordered[0]![0]*r,ordered[0]![1]*r);let exit: Vector|null=null;
  ...
  line(coords: readonly (readonly [number, number])[],r: number): void {const ctx=this.ctx;ctx.beginPath();let last: Vector|null=null;for(const [lon,lat]of coords){const p=this.point(lon,lat);if(p[2]>=0){if(!last||last[2]<0)ctx.moveTo(p[0]*r,p[1]*r);else ctx.lineTo(p[0]*r,p[1]*r);}last=p;}ctx.stroke();}
  draw(t: number): void {
    ...
    for(let lat=-75;lat<=75;lat+=15){const a: [number, number][]=[];for(let lon=-180;lon<=180;lon+=3)a.push([lon,lat]);this.line(a,r);}
    for(let lon=-180;lon<180;lon+=15){const a: [number, number][]=[];for(let lat=-90;lat<=90;lat+=3)a.push([lon,lat]);this.line(a,r);}
  ...
  destroy(): void {cancelAnimationFrame(this.raf);this.observer.disconnect();}
